...
def is_7digit_lowercase(s):
     #   return re.match("^[a-z]*$", s)
        return re.match("^[a-z][a-z][a-z][a-z][a-z][a-z][a-z]$", s)
...
def printbigwaffle(letters,colors):
    for (i,(let,col)) in enumerate(zip(letters,colors)):
        printletter(let,col)
        print(" ",end='')
        if i in {7,8,9,18,19,20,29,30,31}:
             print("  ",end="")
        if i in {6,10,17,21,28,32,39}:
             print()
    print()
...
while True:
    ...
    if len(letters)==0:
        print('using waffle delux #73')
        letters= 'aafuzveaautnnrnaaayelnmemdomhietdienyslt'
        colors = 'ywgygwywyyygygwgwgwggwgwgwgygwwwywwgygwy'
        printbigwaffle(letters,colors)
        break
...
#for ws in wordspaces:
    ...
    rex = ['^','[a-z]','[a-z]','[a-z]','[a-z]','[a-z]','[a-z]','[a-z]','$']
    nongreenletters = []
    for k in range(7):
...
for kk in range(numsoln):
    ...
    if line=='':
        break
    print(line,end='')
    solutionletters = ['.' for i in range(40)]
    for k in range(48):
    ...
    allgreen = ''.join(['w' for iii in range(48)])
    printbigwaffle(solutionletters,allgreen)
